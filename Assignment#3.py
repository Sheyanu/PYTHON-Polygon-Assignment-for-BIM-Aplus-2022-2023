...
Ix = 0
for i in range(num_points):
    Ix += (points_x[i+1] - points_x[i]) * (points_y[i+1] ** 3 + (points_y[i+1] ** 2) * (points_y[i]) + (points_y[i+1] * points_y[i] ** 2) +points_y[i] ** 3)
Ix = -Ix/12

Iy = 0
for i in range(num_points):
    Iy += (points_y[i+1] - points_y[i]) * (points_x[i+1] ** 3 + (points_x[i+1] ** 2) * (points_x[i]) + (points_x[i+1] * points_x[i] ** 2) +points_x[i] ** 3)
Iy = Iy/12
